...
describe('authored views', () => {
  it('extracts typed edits from authored features and ignores malformed ones', () => {
    ...
    doc.authored.features.push(
      {
        ...
        properties: { layer: 'terrainEdit', origin: 'authored', op: 'raise', radiusM: 50, amount: 20 },
      },
      {
        type: 'Feature',
        id: 't2',
        ...
        properties: { layer: 'terrainEdit', origin: 'authored', op: 'explode' },
      },
      {
        type: 'Feature',
        id: 'f1',
        geometry: { type: 'Point', coordinates: [5, 5] },
        properties: { layer: 'fieldEdit', origin: 'authored', field: 'wealth', delta: -0.5 },
      },
      {
        type: 'Feature',
        id: 'z1',
    ...
    expect(
      reseedSalt(
        [
          { op: 'reseed', target: 'x', salt: 'a' },
          { op: 'reseed', target: 'region', salt: 'b' },
        ],
        'x',
      ),
    ).toBe('a+b');
...
describe('terrain edits', () => {
  ...
  it('flows through the terrain stage and memo key', async () => {
    const base: TerrainInput = {
      seed: 'edit',
      extent: { widthM: 6000, heightM: 6000 },
      preset: 'plains',
      relief: 0.3,
      roughness: 0.3,
      seaLevel: 0,
      rivers: { major: 0, minor: 1 },
      cellSizeM: 50,
    };
    const runner = new StageRunner();
    const plain = await runner.run(terrainStage, base);
    const edited = await runner.run(terrainStage, {
      ...base,
      edits: [{ id: 'e', op: 'raise', points: [[0, 0]], radiusM: 300, amount: 80 }],
    });
...
describe('field edits, zone overrides and reseeding', () => {
  it('paints wealth, overrides wards and changes layout with a salt', async () => {
    ...
    const settlements = [
      {
        ...
        kind: 'town' as const,
        population: 4000,
        site: { center: [0, 0] as [number, number], lock: true },
    ...
    const painted = await runner.run(societyStage, {
      ...soc,
      edits: [{ id: 'p', field: 'wealth', points: [[0, 0]], radiusM: 400, delta: -0.6 }],
    });
    ...
    const zoned = await runner.run(townStage, {
      seed: 'hooks',
      site,
      terrain,
      year: 1650,
    ...
    const salted = await runner.run(townStage, {
      seed: 'hooks',
      site,
      terrain,
      year: 1650,
