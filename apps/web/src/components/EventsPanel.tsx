...
export function EventsPanel() {
  ...
  return (
    <section className="space-y-2" data-testid="events">
      <h2 className="text-xs font-semibold uppercase tracking-wide text-stone-500">{t('Disasters')}</h2>
      <div className="flex flex-wrap items-center gap-1 text-xs">
        <select
          aria-label={t('Disaster type')}
          className={sel}
          value={kind}
        ...
        <label className="flex items-center gap-1">
          <span className="text-stone-600">{t('severity')}</span>
          <input
        ...
        {kind === 'flood' && (
          <>
            ...
            <label className="flex items-center gap-1">
              <span className="text-stone-600">{t('years')}</span>
              <input
                aria-label={t('Flood years')}
                type="number"
                className="w-14 rounded border border-stone-300 px-1"
                value={duration}
                min={1}
        ...
        <button
          ...
          onClick={add}
          title={t('At the centre of the map view, in the current year')}
        >
        ...
        <ul className="space-y-0.5 text-[11px] text-stone-600" data-testid="events-list">
          {events.map((e, i) => (
            <li key={e.id} className="flex items-center justify-between gap-1">
              <span>
                {t('{kind} of {year}', { kind: t(KIND_LABELS[e.kind]), year: e.year })}
              ...
              <button
                ...
                aria-label={t('Remove {kind} of {year}', { kind: t(KIND_LABELS[e.kind]), year: e.year })}
                onClick={() =>
